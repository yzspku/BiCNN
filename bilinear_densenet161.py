...
class BCNN(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.in_features = torchvision.models.densenet161().classifier.in_features #2208
    ...
    def forward(self, X):
        ...
        X = self.features(X)
        assert X.size() == (N, self.in_features, 7, 7)
        X = self.conv(X)
        X = self.bn(X)
        X = self.relu(X)
        X = X.view(N, 512, 7**2)
    ...
    def __init__(self, options, path, freeze=True, pre_model_path=None):
        ...
        if freeze is True:
            self._solver = torch.optim.SGD(
                list(self._net.module.conv.parameters())+list(self._net.module.bn.parameters())+list(self._net.module.fc.parameters()),
...
def fc():
    ...
    pre_model_path = None
    path_save='models/densenet152_fc.pth'
...
def all_layers():
    ...
    pre_model_path = 'models/densenet152_fc.pth'
    path_save='models/densenet152_all.pth'
